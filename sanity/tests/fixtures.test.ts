...
describe('Fixture Data Validation', () => {
  ...
  describe('Site Settings Fixture', () => {
    test('should have valid site settings data', () => {
      expect(siteSettingsFixture._type).toBe('siteSettings')
      expect(siteSettingsFixture.title).toBeTruthy()
      expect(siteSettingsFixture.description).toBeTruthy()
      
      if (siteSettingsFixture.mainNav) {
        expect(Array.isArray(siteSettingsFixture.mainNav)).toBe(true)
        siteSettingsFixture.mainNav.forEach(navItem => {
          expect(navItem.title).toBeTruthy()
          expect(navItem.href).toBeTruthy()
        })
      }
      
      if (siteSettingsFixture.socialLinks) {
        expect(Array.isArray(siteSettingsFixture.socialLinks)).toBe(true)
        siteSettingsFixture.socialLinks.forEach(link => {
