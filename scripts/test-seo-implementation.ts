...
const hreflangPath = join(process.cwd(), 'components', 'seo', 'HreflangTags.tsx')
if (existsSync(hreflangPath)) {
  console.log('✅ HreflangTags component exists')
  const content = readFileSync(hreflangPath, 'utf-8')
  if (content.includes('generateHreflangMetadata') && content.includes('withHreflang')) {
...
const countrySelectorPath = join(process.cwd(), 'components', 'layout', 'CountrySelector.tsx')
if (existsSync(countrySelectorPath)) {
  ...
  if (content.includes('HiGlobeAlt') && content.includes('US') && content.includes('AU/NZ')) {
...
const clientHeaderPath = join(process.cwd(), 'components', 'layout', 'ClientHeader.tsx')
const clientFooterPath = join(process.cwd(), 'components', 'layout', 'ClientFooter.tsx')
