// Generated from components/newsroom/NewsroomPageContent.tsx. Run npm run localize:resources; do not edit directly.
'use client'
...
interface NewsroomPageContentProps {
  posts: NewsPost[]
  currentPage?: number
}

export default function NewsroomPageContent({ posts, currentPage = 1 }: NewsroomPageContentProps) {
        ...
        <div className="max-w-2xl mx-auto mb-12">
          ...
          {searchQuery && (
            <p className="mt-3 text-sm text-neutral-600 text-center">
              Found {filteredPosts.length} {filteredPosts.length === 1 ? 'news post' : 'news posts'} 
        ...
        {paginatedPosts && paginatedPosts.length > 0 ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {paginatedPosts.map((post) => (
                <BlogCard basePath="/us/blog" key={post._id} post={post} />
              ))}
            </div>
          ...
          <div className="text-center py-12">
            <h2 className="text-2xl font-semibold text-neutral-600 mb-4">
              {searchQuery ? 'No news posts found' : 'No news posts yet'}
            </h2>
