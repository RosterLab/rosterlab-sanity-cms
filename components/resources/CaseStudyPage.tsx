...
export function createCaseStudyPage(isUS = false) {
  interface CaseStudyPageProps {
    params: Promise<{
      slug: string;
    }>;
  }
  const caseStudyQuery = isUS
    ? groq`
...
`
    : groq`
  *[_type == "post" && (!defined(sites) || sites != "us") && slug.current == $slug && "case-studies" in categories[]->slug.current][0] {
    _id,
    _updatedAt,
    sites,
    title,
...
`;
  const caseStudyPathsQuery = isUS
    ? groq`
  *[_type == "post" && (!defined(sites) || sites != "global") && "case-studies" in categories[]->slug.current][]{"slug": slug.current, "usSlug": usSlug.current}
`
    : groq`
  *[_type == "post" && (!defined(sites) || sites != "us") && "case-studies" in categories[]->slug.current][].slug.current
`;
  const allCaseStudiesQuery = isUS
    ? groq`
...
`
    : groq`
  *[_type == "post" && (!defined(sites) || sites != "us") && "case-studies" in categories[]->slug.current] | order(publishedAt desc) {
    _id,
    title,
    slug,
    excerpt,
...
`;
  ...
  async function generateMetadata({ params }: CaseStudyPageProps) {
    ...
    const post = await (isUS
      ? clientToUse
          .fetch(caseStudyQuery, { slug: globalizeUSSlug(slug), usSlug: slug })
    ...
    if (!post) {
      return resourceMetadata(
        ...
        isUS ? `/us/case-studies/${slug}` : `/case-studies/${slug}`,
      );
    }
    const baseUrl = isUS
      ? "https://rosterlab.com"
    ...
    if (!isUS) {
      if (slug === "icu-unit-western-australia") {
        metaDescription =
          "Western Australian ICU transforms staff rostering with RosterLab, achieving significant time savings and improved staff satisfaction.";
      } else if (metaDescription.length < 120) {
        // If description is too short, enhance it
        metaDescription =
          `${metaDescription} Read the full case study on how RosterLab transforms healthcare workforce management.`.slice(
            0,
            155,
          );
    ...
    return resourceMetadata(
      {
        ...
        alternates: {
          canonical: isUS
            ? `${baseUrl}/us/case-studies/${slug}`
            : `${baseUrl}/case-studies/${slug}`,
        },
        openGraph: {
          title: post.seo?.metaTitle || post.title,
          description: metaDescription,
          type: "article",
          url: isUS
            ? `https://rosterlab.com/us/case-studies/${slug}`
            : `https://rosterlab.com/case-studies/${slug}`,
  ...
  async function CaseStudyPage({ params }: CaseStudyPageProps) {
    const { slug } = await params;
    const { isEnabled } = await draftMode();
    const clientToUse = getClient(
      isEnabled && validatedToken ? { token: validatedToken } : undefined,
    ...
    return (
      <CaseStudyGateCheck isUS={isUS ? true : undefined} slug={slug}>
        <article>
          <BlogPostTracker
            title={post.title}
                        ...
                        shouldShowLastUpdated(
                          ...
                          <>
                            ...
                            <time className="text-purple-200">
                              Last Updated:{" "}
                              {isUS
                                ? formatDateShort(post._updatedAt, "en-US")
              ...
              <div className="py-2">
                <Breadcrumb
                  items={[
                    ...
                    {
                      label: "Case Studies",
                      href: isUS ? "/us/case-studies" : "/case-studies",
                    },
                    { label: post.title },
                  ]}
                />
  ...
  return { Page: CaseStudyPage, generateMetadata, generateStaticParams };
}
