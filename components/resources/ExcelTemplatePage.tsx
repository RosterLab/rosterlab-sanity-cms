...
export function createExcelTemplatePage(isUS = false) {
  const recommendedPostsQuery = isUS
    ? groq`
...
`;
  const features = [
    {
      icon: HiTable,
      title: isUS
        ? "Pre-formatted 6-week schedule"
        : "Pre-formatted 6-week roster",
      description:
  ...
  const benefits = [
    ...
    isUS
      ? "Edit and personalize to meet your requirements"
  ...
  const faqItems = [
    {
      ...
      answer: isUS
        ? "This free Excel staff schedule template is designed for managers, staff schedulers and team leaders who need a simple way to organize staff shifts. It's best suited for workplaces that want a straightforward, no-cost solution without needing to adopt new software."
    ...
    {
      question: isUS
        ? "Should I use an Excel spreadsheet to schedule my staff?"
        : "Should I use an Excel spreadsheet to roster my staff?",
      answer:
    ...
    },
    {
      ...
      answer: isUS
        ? "The Excel template is static - you download it and update manually. Our <a href='/us/solutions/free-staff-scheduling-tool' class='text-blue-600 hover:text-blue-700 underline'>free digital scheduling tool</a>, on the other hand, allows you to build and share schedules online, make real-time updates, and automatically notify staff of changes. Digital scheduling reduces admin time, helps prevent double-ups, and ensures your team always has the latest version."
  ...
  async function FreeStaffRosterTemplateExcelPage() {
    const { isEnabled } = await draftMode();
    const client = getClient(
      isEnabled && validatedToken ? { token: validatedToken } : undefined,
    );
    const recommendedPosts = await (isUS
      ? client.fetch(recommendedPostsQuery).then(localizeUSResourceResult)
      : client.fetch(recommendedPostsQuery));
    return (
      <SiteLayout>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                <div>
                  ...
                  <h1 className="text-[40px] sm:text-5xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-6 leading-tight">
                    {isUS ? (
                      <>Free Staff Schedule Template (Excel)</>
                    ) : (
                  ...
                  <p className="text-xl text-gray-600 mb-8">
                    {isUS ? (
                      <>
                        Stop struggling with schedule creation. Download our
                        professionally designed Excel template specifically
                        built for shift work scheduling.
                      </>
                    ) : (
                      <>
                ...
                <ExcelFormClient isUS={isUS ? true : undefined} />
              </div>
            </Container>
          </div>
            ...
            <Container>
              <div className="text-center mb-12">
                ...
                <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                  {isUS ? (
                    <>
                      Everything you need to create professional staff schedules
                      in Excel
                    </>
                  ) : (
                    <>
                      Everything you need to create professional staff rosters
                      in Excel
                    </>
                  )}
                </p>
                ...
                <div className="bg-white rounded-lg shadow-lg p-8 overflow-hidden">
                  <div className="relative overflow-hidden rounded">
                    <Image
                      ...
                      alt={
                        isUS
                          ? "Excel schedule template preview showing a 6-week calendar with staff names, shift allocations, and automatic totals"
                          : "Excel roster template preview showing a 6-week calendar with staff names, shift allocations, and automatic totals"
                      }
                      width={1200}
                      height={600}
          ...
          <section className="bg-white py-20">
            <Container>
              <h2 className="text-3xl font-bold text-center text-gray-900 mb-12">
                {isUS ? (
                  <>Recommended reading for Excel Schedulers</>
                ) : (
            ...
            <Container>
              <div className="max-w-4xl mx-auto">
                ...
                <p className="text-center text-gray-600 mb-12 text-lg">
                  {isUS ? (
                    <>
                      Everything you need to know about our Excel schedule
                      template
                    </>
                  ) : (
                    <>
                      Everything you need to know about our Excel roster
                      template
                    </>
                  )}
                </p>
            ...
            <Container>
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-3xl p-12 text-center text-white">
                <h2 className="text-3xl font-bold mb-4">
                  {isUS ? (
                    <>Still Creating Schedules Manually?</>
                  ) : (
                ...
                <p className="text-xl mb-8 opacity-90 max-w-2xl mx-auto">
                  {isUS ? (
                    <>
                      While our Excel template saves time, RosterLab's AI can
                      create optimized schedules in minutes, not hours. See how
                      much time you could save.
                    </>
                  ) : (
                    <>
                      While our Excel template saves time, RosterLab's AI can
                      create optimized rosters in minutes, not hours. See how
                      much time you could save.
                    </>
                  )}
                </p>
  ...
  return FreeStaffRosterTemplateExcelPage;
}
