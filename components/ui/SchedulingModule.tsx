  ...
  const startOptimization = () => {
    ...
    setTimeout(() => {
      ...
      optimizedStaff.forEach((optimizedMember) => {
        const originalMember = initialStaff.find(
          (m) => m.id === optimizedMember.id,
        );
        if (originalMember) {
      ...
      <div className="p-6">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                <th className="text-left py-3 px-4 font-semibold text-gray-700 bg-gray-50 rounded-l-lg">
                  Staff Member
                </th>
                {daysOfWeek.map((day) => (
                  <th
                    key={day}
                    className="text-center py-3 px-2 font-semibold text-gray-700 bg-gray-50 last:rounded-r-lg"
                  >
                    {day}
                  </th>
                ))}
              ...
              {staff.map((member, index) => (
                ...
                >
                  ...
                  {daysOfWeek.map((day) => {
                    ...
                    const isAnimating = animatingCells.has(
                      `${member.id}-${day}`,
                    );
                    return (
