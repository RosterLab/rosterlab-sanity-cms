  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-8">
      <div className="flex gap-6 items-start">
                ...
                <div className="flex-1">
                  <div className="flex h-12 rounded-lg overflow-hidden shadow-sm">
                    {[1, 2, 3].map((week) => (
                      <div
                        key={week}
                        className={`flex-1 flex items-center justify-center font-medium text-sm transition-all duration-500 ${
                      ...
                      >
                        {profile.weekendsOff.includes(week) ? `Week ${week}` : ''}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* Right side - Benefits */}
        <div className={`rounded-lg p-5 flex-[1.2] transition-all duration-500 ${
          isOptimized ? 'bg-gray-50' : 'bg-gray-100'
        }`}>
          <h4 className={`font-semibold mb-4 transition-colors duration-500 ${
