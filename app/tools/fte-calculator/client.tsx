...
function fmt(n: number, dp: number, isUS = false): string {
  return n.toLocaleString(isUS ? "en-US" : "en-NZ", {
...
export default function FTECalculatorClient({
  ...
  const stampPrintDate = useCallback(() => {
    setPrintDate(
      new Date().toLocaleDateString(isUS ? "en-US" : "en-NZ", {
        day: "numeric",
        month: "long",
        year: "numeric",
      }),
  ...
  const generatePdfSnapshot = useCallback(async () => {
    ...
    const dateStr = new Date().toLocaleDateString(isUS ? "en-US" : "en-NZ", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
    ...
    const summary: [string, string][] = [
      [
        "Total weekly demand",
        `${fmt(totalHours, totalHours % 1 ? 1 : 0, isUS)} hrs`,
      ],
      ["Hours per FTE", hoursPerFte || "—"],
      ["Scaling factor", `${fmt(scaleClean, scaleClean % 1 ? 1 : 0, isUS)}%`],
      ["Required FTE", requiredFte === null ? "—" : fmt(requiredFte, 2, isUS)],
    ...
    doc.text(
      isUS
        ? "Generated by the RosterLab FTE Requirement Calculator · rosterlab.com/us/tools/fte-calculator"
  ...
  const requiredFteDisplay =
    requiredFte === null ? "—" : fmt(requiredFte, 2, isUS);
      ...
      <Container className="max-w-[1400px]">
        <section className="mb-6 sm:mb-7">
          ...
          <p className="mt-2.5 text-sm sm:text-[15px] text-[#55606B] max-w-none">
            {isUS ? (
              <>
                Enter each shift in your demand model with its times and how
                many staff you need on each day of the week. We&apos;ll total
                the weekly hours and convert them into the full-time equivalents
                (FTE) required to cover the schedule.
              </>
            ) : (
              <>
                Enter each shift in your demand model with its times and how
                many staff you need on each day of the week. We&apos;ll total
                the weekly hours and convert them into the full-time equivalents
                (FTE) required to cover the roster.
              </>
            )}
          </p>
          {printDate && (
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5 mt-6 sm:mt-7">
          <div className="bg-white border border-[#E4E8EC] border-t-4 border-t-[#24D9DC] rounded-[10px] p-4 sm:p-5 shadow-[0_2px_10px_rgba(26,26,46,0.05)]">
            ...
            <div className="text-[28px] sm:text-[34px] font-semibold leading-tight">
              {fmt(totalHours, totalHours % 1 ? 1 : 0, isUS)}{" "}
          ...
          <div className="bg-white border border-[#E4E8EC] border-t-4 border-t-[#24D9DC] rounded-[10px] p-4 sm:p-5 shadow-[0_2px_10px_rgba(26,26,46,0.05)]">
                ...
                >
                  {isUS ? (
                    <>
                      ...
                      demand exactly as entered. Since staff take vacation, sick
                      leave and training, the FTE you employ typically needs to
                      be higher. A common starting point is 15–20% — tune it to
                      your team&apos;s historical leave and absence rates.
                    </>
                  ) : (
                    <>
                      With a 0% scaling factor, this is the FTE needed to cover
                      demand exactly as entered. Since staff take annual leave,
                      sick leave and training, the FTE you employ typically
                      needs to be higher. A common starting point is 15–20% —
                      tune it to your team&apos;s historical leave and absence
                      rates.
                    </>
                  )}
            ...
            <div className="text-xs font-light text-[#6B7280] mt-1.5">
              {isUS ? (
                <>
                  Uplift for vacation, sick leave, training and other systemic
                  inefficiency
                </>
              ) : (
                <>
                  Uplift for annual leave, sick leave, training and other
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mt-4 sm:mt-5">
          <div className="bg-white border border-[#E4E8EC] border-t-4 border-t-[#24D9DC] rounded-[10px] p-4 sm:p-5 shadow-[0_2px_10px_rgba(26,26,46,0.05)]">
            ...
            <div className="text-xs font-light text-[#6B7280] mt-1.5">
              {isUS ? (
                <>
                  The FTE you currently have (or plan to have) on this schedule
                </>
              ) : (
        ...
        <section className="mt-10 sm:mt-14 print:hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            <Link
              href={
                isUS ? "/us/tools/savings-calculator" : "/tools/roi-calculator"
              }
            ...
            >
              <div className="relative h-40 bg-gradient-to-br from-[#E6FDF8] to-[#F7F9FB]">
                <Image
                  ...
                  alt={isUS ? "Savings Calculator" : "ROI Calculator"}
                  fill
            ...
            >
              <div className="relative h-40 bg-gradient-to-br from-[#E6FDF8] to-[#F7F9FB]">
                <Image
                  ...
                  alt={
                    isUS
                      ? "Scheduling as a Strategic Workforce Lever whitepaper"
                      : "Rostering as a Strategic Workforce Lever whitepaper"
                  }
                  fill
                  className="object-cover"
                />
              ...
              <div className="p-5">
                ...
                <p className="text-sm text-[#55606B]">
                  {isUS ? (
                    <>
                      How healthcare executives turn scheduling into a lever for
            ...
            >
              ...
              <div className="relative p-6 flex flex-col h-full">
                ...
                <h3 className="text-xl font-semibold mb-4 max-w-[85%]">
                  {isUS ? (
                    <>Turn your FTE requirement into an optimal schedule</>
                  ) : (
