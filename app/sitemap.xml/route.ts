...
function hasNoIndex(pagePath: string): boolean {
  try {
    ...
    try {
      ...
      if (
        layoutContent.includes("robots:") &&
        layoutContent.includes("index: false")
      ) {
        return true;
      }
    } catch {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        if (
          excludedPaths.some((excluded) => currentPath.startsWith(excluded))
        ) {
          continue;
        }
...
async function generateSitemap() {
  ...
  const blogPosts =
    posts?.filter((post: any) => {
      const categorySlugStrings =
        post.categories?.map((cat: any) => cat.slug?.current) || [];
      return (
        !categorySlugStrings.includes("case-studies") &&
        !categorySlugStrings.includes("newsroom")
      );
    }) || [];
