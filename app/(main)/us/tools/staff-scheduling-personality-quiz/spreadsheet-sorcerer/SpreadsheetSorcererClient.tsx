...
export default function SpreadsheetSorcererClient({ recommendedPosts }: SpreadsheetSorcererClientProps) {
  ...
  const generatePDF = useCallback(async () => {
    ...
    doc.setFontSize(10)
    doc.setTextColor(...textColor as [number, number, number])
    const sorcererDesc = 'You wield formulas and functions like a wizard casts spells, creating schedules that are both efficient and elegant.'
    ...
    doc.setTextColor(...textColor as [number, number, number])
    doc.text(': Real-time schedule access for all staff', 28 + tool1Width, currentY)
    currentY += 8
    
    // Tool 2: Automation schedule generation
    doc.setTextColor(...textColor as [number, number, number])
    doc.text('• ', 25, currentY)
    doc.setTextColor(...linkBlue as [number, number, number])
    ...
    doc.link(28, currentY - 3, tool2Width, 4, {url: 'https://rosterlab.com/us/solutions/ai-staff-scheduling'})
    doc.setTextColor(...textColor as [number, number, number])
    ...
    doc.link(28, currentY - 3, tool3Width, 4, {url: 'https://rosterlab.com/us/feature/shift-swaps'})
    doc.setTextColor(...textColor as [number, number, number])
    ...
    doc.link(28, currentY - 3, blog3Width, 4, {url: 'https://rosterlab.com/us/blog/how-to-implement-self-scheduling'})
    currentY += 17
          ...
          <div className="grid gap-8 md:grid-cols-3">
            <Link href="/us/solutions/staff-scheduling-mobile-app" className="block">
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-lg h-full">
                ...
                <p className="text-gray-600 mb-4">
                  Take your perfectly crafted schedules mobile. Staff can view schedules, request changes, and stay connected on the go.
                </p>
            ...
            <Link href="/us/solutions/ai-staff-scheduling" className="block">
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-lg h-full">
                ...
                <p className="text-gray-600 mb-4">
                  Combine your spreadsheet expertise with AI power. Generate optimized schedules in seconds while maintaining control.
                </p>
                <span className="text-primary-600 font-medium inline-flex items-center">
                  Learn more 
                  <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
